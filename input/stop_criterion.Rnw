...
One way to test for such a compound hypothesis is to test each of the $m$ \sidenote{Let $m$ be the number of covariates} hypothesis seperately and to reject the global null hypothesis if one of the covariates and $Y$ are significantly dependent. To overcome the problem of multiple testing a p-value correction has to take place. The whole procedure:  
