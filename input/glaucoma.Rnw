...
The data for illustration is available in the \lstinline{ipred} package (\citet{ipred}). 
...
%% mean 
\[\mu_j = \sum\limits_{i:node} X_{ji} \frac{1}{n_{node}} \begin{pmatrix} n_{Glaucoma} \\ n_{normal} \end{pmatrix}  = \begin{pmatrix} n_{Glaucoma} \cdot \bar{X}_{j, node} \\ n_{normal} \cdot \bar{X}_{j, node} \end{pmatrix} \]
%% variance
\[\Sigma = 
...
\underbrace {\frac{1}{n_{node} - 1}\sum\limits_{i:node} \left( X_{ji} - \bar{X}_{j, node} \right)^2}_{\widehat{Var}(X_{j, node})} \\
\end{align*}
...
Thus
\[ c = n_{Class}  \frac{ \bar{X}_{j, Class} - n_{Class} \bar{X}_{j, node}}{\sqrt{(\Sigma)_{kk}}} = 
