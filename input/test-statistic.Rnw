...
is derived from  \citet{strasser} and can be used to test if a response $Y$ and a covariate $X$ are independent.  
...
is to take the maximum of the standardized test statistic. This yields the following standardized linear test statistic: \sidenote{see page \pageref{chap:strasser} for the calculation of $\mu$ and $\Sigma$ of $T$}
