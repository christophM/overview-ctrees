...
are a special class of non-parametric tests\citep{permutation}.
...
The number of permutations is $n!$ and the calculation time for all possible test statistics is for most problems unreasonably long. 
