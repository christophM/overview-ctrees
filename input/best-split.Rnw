...
with $A$ beeing a possible partition of the current observations and 
\[I(X_{ji} \in A) = \begin{cases} 1, & X_{ij} \in A \\ 0 & X_{ij} \not\in A \end{cases}\]
...
An appropriate function to capture only the difference in the partition, the transformation of $X$ is the indicator function. 
This results in the statistic 
\[c_{max}(\mathbf{t}, \mu, \Sigma) = max_{k} \left| \frac{(\mathbf{t}^A-\mu)_k}{\sqrt{(\Sigma)_{kk}}}\right|\]
Note that the test statistic does not depend on the transformation $g_j$ of $X_j$. The scale of the chosen covariate $X_j$ is
...
We search the $A$ which maximizes:\marginnote{\includegraphics[width = 5cm]{images/partition.png}}
